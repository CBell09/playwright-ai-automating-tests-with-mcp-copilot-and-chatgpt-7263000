...
test.describe('Delete Bug - cancel via Escape key', () => {
  let bugId: number;
  let title: string;
  ...
  test.beforeEach(async ({ page, request, loginPage }) => {
    ...
    title = `escape-${randomUUID().slice(0, 8)}`;
  ...
  test('should keep the bug and return to the edit modal when Escape is pressed on the confirmation modal', async ({
    page,
    boardPage,
    editBugModal,
    ...
    await confirmDeleteBugModal.pressEscape();
