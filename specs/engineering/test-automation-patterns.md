...
When a workflow spans multiple pages, use multiple page object fixtures in the test:

```typescript
test('user creates a bug and views it on the board', async ({ boardPage, createBugModal }) => {
  // Act
...
Adding a new page object means adding one fixture to this file — no new file, no separate merge step.
...
| Constructing page objects with `new` in a test or helper | Consume the matching page object fixture from `../fixtures/pages` |
